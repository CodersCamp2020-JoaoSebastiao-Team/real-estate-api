...
import * as chargeHelper from '../handlers/chargeHelper';
...
export class ListingController {
  ...
  public create_listing(req: Request, res: Response) {
    ...
    if (req.body) {
      ...
      this.listing_service.createListing(
        listing_params,
  ...
  public get_listing(req: Request, res: Response) {
    if (req.params.id) {
      ...
      this.listing_service.filterListings(
        listing_filter,
        (err: any, listing_data: IListing) => {
          if (err) {
            mongoError(err, res);
  ...
  private filters(req: Request, listing_filter: any) {
    ...
    if (req.query.listingStatusType) {
      listing_filter = listing_filter["listingStatusType"] =
        req.query.listingStatusType;
    }
  ...
  public get_all_listings(req: Request, res: Response) {
    ...
    this.listing_service.findAllListings(
      listing_filter,
      (err: any, listing_data: IListing) => {
    ...
    this.listing_service.findAllListings(
      ...
      (err: any, listing_data: IListing) => {
        if (err || listing_data === null) {
          ...
          successResponse(
            "get available listings successfully",
            listing_data,
            res
          );
        }
  ...
  public delete_listing(req: Request, res: Response) {
    if (req.params.id) {
      this.listing_service.deleteListing(
        req.params.id,
  ...
  public createChargeStripe(req: Request, res: Response, next: any) {
    // Token is created using Checkout or Elements!
    // Get the payment token ID submitted by the form:
    const stripeToken = req.body.stripeToken;
    const price = chargeHelper.getPrice(req.body.order);
